...
export default function DeleteGoal({id, toggleModal} : Props) {
    ...
    const deleteGoal = async () : Promise<void> => {
        ...
        try {
            await progressive_overloading.execAsync(`DELETE FROM goals WHERE id = ${id}`);
            setRefreshGoalsDatabase( prev => ! prev)
            ToastAndroid.show("Goal deleted", ToastAndroid.LONG)
        } catch (error) {
        ...
        <View style={styles.modalContainer}>
            <View style={[styles.popup_container, { backgroundColor: colorScheme === 'dark' ? '#333' : '#fff' }]}>
                ...
                <View style={styles.button_views}>
                    <TouchableOpacity onPress={deleteGoal} style={[styles.delete_button, styles.both_button]}>
